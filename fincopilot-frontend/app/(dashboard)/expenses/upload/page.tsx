...
export default function UploadPage() {
  ...
  return (
    <div className="p-6 max-w-xl animate-fade-in">
      ...
      <div className="mb-6">
        <h1 className="text-xl font-bold text-slate-900 dark:text-white tracking-tight">Import Transactions</h1>
      ...
      <div
        ...
        className={`border-2 border-dashed rounded-2xl p-12 text-center cursor-pointer transition-all duration-200 ${
          isDragActive
            ...
            : "border-slate-900/10 dark:border-white/10 hover:border-violet-500/40 hover:bg-slate-900/[0.03] dark:hover:bg-white/[0.03]"
        } ${(uploading || jobId) ? "opacity-50 cursor-not-allowed pointer-events-none" : ""}`}
      >
        <input {...getInputProps()} />
        <div className="w-12 h-12 rounded-2xl bg-slate-900/[0.05] dark:bg-white/[0.05] border border-slate-900/[0.09] dark:border-white/[0.08] flex items-center justify-center mx-auto mb-4">
          <Upload size={20} className={isDragActive ? "text-violet-600 dark:text-violet-400" : "text-slate-500"} />
        </div>
      ...
      {job && (
        <div className="mt-6 glass rounded-2xl p-5">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <p className="font-semibold text-slate-900 dark:text-slate-100 text-sm capitalize">{job.status}</p>
              <p className="text-xs text-slate-500">{job.progress}% complete</p>
            </div>
          </div>

          <div className="w-full bg-slate-900/[0.06] dark:bg-white/[0.06] rounded-full h-1.5 mb-4 overflow-hidden">
            <div
          ...
          <div className="flex gap-6 text-xs">
            <div className="text-slate-500">Created: <strong className="text-emerald-600 dark:text-emerald-400">{job.created}</strong></div>
            <div className="text-slate-500">Failed: <strong className="text-rose-600 dark:text-rose-400">{job.failed}</strong></div>
          </div>
