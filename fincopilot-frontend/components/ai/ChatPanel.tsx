  ...
  return (
    <div className="fixed bottom-6 right-6 w-96 h-[520px] flex flex-col z-50 animate-slide-in-right">
      ...
      >
        ...
        {/* Offline banner */}
        {disabled && (
          <div className="flex items-center gap-2 px-4 py-2.5 text-xs text-amber-300 bg-amber-500/10 border-b border-amber-500/20">
            <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-amber-400" />
            <span>
              <strong className="text-amber-200">AI offline.</strong>{" "}
              ...
              <div
                className={`max-w-[80%] rounded-2xl px-3.5 py-2.5 text-sm leading-relaxed ${
                  msg.role === "user"
                    ...
                    : msg.isError
                    ? "bg-amber-500/10 text-amber-200 rounded-bl-sm border border-amber-500/20"
        ...
        <div className="px-3 py-3 border-t border-white/[0.06]">
          <div className="flex gap-2">
            <textarea
              ...
              rows={1}
              className="flex-1 resize-none bg-white/[0.05] border border-white/[0.08] focus:border-violet-500/50 rounded-xl px-3 py-2 text-sm text-slate-200 placeholder-slate-600 focus:outline-none focus:bg-white/[0.07] transition-all min-h-[38px] max-h-24 disabled:cursor-not-allowed disabled:opacity-50"
            />
            <button
              onClick={handleSend}
